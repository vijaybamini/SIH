        ...
        <div className="profile-page-inner">
          ...
          <div className="profile-summary-header">
            ...
            <div className="profile-summary-header-text">
              ...
              <h2>{t.serviceProviderProfileTitle}</h2>
            </div>
          ...
          <div className="summary-section">
            <h3>{t.millDetailsWord}</h3>
            {mills.length === 0 ? (
              <div className="empty-card"><p>{t.noMillsYet}</p></div>
            ) : mills.map((mill, index) => (
              <div className="summary-crop-card" key={mill.id}>
                <strong>{t.millNumber.replace('{n}', index + 1)}</strong>
                <div className="summary-grid">
                  <div><span>{t.cropServices}</span><strong>{mill.cropTypes.filter(Boolean).join(', ') || '—'}</strong></div>
                  <div><span>GSTIN</span><strong>{mill.gstin || '—'}</strong></div>
                </div>
                {mill.documentUrl && <p className="form-note"><a href={mill.documentUrl} target="_blank" rel="noreferrer">📄 {t.viewUploadedDocument}</a></p>}
              </div>
            ))}
          </div>
        </div>
  ...
  return (
    <div className="profile-page">
      <div className="profile-page-inner">
        ...
        <h2>{t.serviceProviderProfileDetailsTitle}</h2>
        <p className="panel-subtitle">{t.serviceIntroSub}</p>
        ...
        <form className="profile-form-card" onSubmit={handleSubmit}>
          ...
          <h3 className="form-section-title">{t.millDetailsWord}</h3>
          <label>{t.howManyMills}
            <input
              type="number"
              min="1"
              max="20"
              ...
              {mills.map((mill, index) => (
                <div className="crop-card" key={mill.id}>
                  <div className="crop-card-header">
                    <label>{t.millCropServiceInline.replace('{n}', index + 1)}
                  ...
                  {mill.cropTypes.slice(1).map((crop, cropIndex) => (
                    <div className="crop-card-header" key={cropIndex}>
                      ...
                      <button type="button" className="remove-crop-button" onClick={() => removeMillCropType(mill.id, cropIndex + 1)} aria-label={t.removeCropServiceLabel}>×</button>
                    </div>
                  ))}

                  <button type="button" className="add-crop-button" onClick={() => addMillCropType(mill.id)}>
                    <span aria-hidden="true">+</span> {t.addAnotherCropService}
                  </button>
                  ...
                  <div className="form-grid">
                    ...
                    <label>{t.gstinDocument}
                      <div className="doc-upload-box">
                        {mill.documentUrl ? (
                          <span className="doc-upload-filename">📄 {t.documentUploadedReplace}</span>
                        ) : (
                          <span className="doc-upload-placeholder"><span className="photo-upload-icon">📄</span> {t.uploadGstinCertificate}</span>
                        )}
