...
begin
  ...
  case new.raw_user_meta_data ->> 'role'
    when 'farmer' then
      ...
      values (new.id, new.raw_user_meta_data -> 'registration_details' ->> 'land_details', new.raw_user_meta_data -> 'registration_details' ->> 'crop_location', nullif(new.raw_user_meta_data -> 'registration_details' ->> 'area_of_crop', '')::numeric, new.raw_user_meta_data -> 'registration_details' ->> 'survey_number');
    when 'buyer' then
...
insert into public.farmers (profile_id, farm_name, primary_crops)
select id, registration_details ->> 'farm_name', registration_details ->> 'primary_crops'
from public.profiles where role = 'farmer'
on conflict (profile_id) do nothing;

insert into public.farmer_profiles (farmer_id, land_details, crop_location, area_of_crop, survey_number)
select id, registration_details ->> 'land_details', registration_details ->> 'crop_location', nullif(registration_details ->> 'area_of_crop', '')::numeric, registration_details ->> 'survey_number'
from public.profiles where role = 'farmer'
on conflict (farmer_id) do nothing;

insert into public.bulk_buyers (profile_id, business_name, business_type, gstin)
select id, registration_details ->> 'business_name', registration_details ->> 'business_type', registration_details ->> 'gstin'
from public.profiles where role = 'buyer'
on conflict (profile_id) do nothing;

insert into public.logistics_providers (profile_id, company_name, service_areas, fleet_details)
select id, registration_details ->> 'company_name', registration_details ->> 'service_areas', registration_details ->> 'fleet_details'
from public.profiles where role = 'logistics'
on conflict (profile_id) do nothing;

insert into public.service_providers (profile_id, business_name, service_category, service_areas)
select id, registration_details ->> 'business_name', registration_details ->> 'service_category', registration_details ->> 'service_areas'
from public.profiles where role = 'service'
on conflict (profile_id) do nothing;
